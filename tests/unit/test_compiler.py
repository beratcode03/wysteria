...
def test_compile_invalid_proposal():
    prop = WorkflowProposal(
        proposal_version=1,
        workflow={"inputs": {}, "nodes": [{"id": "bad", "kind": "unknown"}], "outputs": {}},
    )
    result = compile_proposal(prop)
...
def test_compiler_determinism():
    ...
    assert json.dumps(normalize_workflow(r1.workflow), sort_keys=True) == json.dumps(
        normalize_workflow(r2.workflow), sort_keys=True
    )
